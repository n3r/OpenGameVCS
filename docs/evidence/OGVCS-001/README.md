...
**Status:** Completed; three-platform conformance and reference scale passed
...
- The frozen source candidate is commit [`d145faf1ed32d8bf6d3cfcd4e6433dc3384c9823`](https://github.com/n3r/OpenGameVCS/commit/d145faf1ed32d8bf6d3cfcd4e6433dc3384c9823).
...
- GitHub Actions [run 31863924721](https://github.com/n3r/OpenGameVCS/actions/runs/31863924721) and its checked-in [machine-readable record](github-actions-run-31863924721.json) bind the final Windows/macOS/Ubuntu evidence to commit `d145faf1ed32d8bf6d3cfcd4e6433dc3384c9823`.

## Presubmit and package results

| Environment | Command | Result |
|---|---|---|
| macOS 26.6.1 arm64, Node 24.9.0, exact frozen product source plus completion records | `npm test` | Fixture suite: 128 passed, 0 failed, 2 intentional skips in 116.453 s; roadmap suite: 7 passed, 0 failed in 0.745 s |
...
| GitHub-hosted Windows, macOS, and Ubuntu, Node 22 | [Workflow run 31863924721](https://github.com/n3r/OpenGameVCS/actions/runs/31863924721) | All three full suites, all golden-summary generations/uploads, the Windows-only portable-path/junction step, and both byte-for-byte comparison steps passed; independently downloaded summary content has SHA-256 `ca0949b76f9dcfb409148a41b588d4911af4c5b2135bc65a73821c193c61adb0` on every platform |
...
The scale request digest is `4bc6dd10cbf46657be264a6bd60dcc86caa1694a78b9a8d3d0fb2db391254bff`; the resulting manifest digest is `c05746cf75850d14e1d64de8ed17baffa9e432bab55c57051ab86be8ff094d55`. The scale run is macOS evidence for AC-03; Windows scale execution is not required by that criterion.

## Acceptance map

| Criterion | Current verdict | Evidence |
|---|---|---|
| OGVCS-001-AC-01 | Passed on Windows, macOS, and Linux | Full CLI profile matrix and offline packed-install test in the presubmit and hosted-run results above |
| OGVCS-001-AC-02 | Passed | The hosted Windows/macOS/Ubuntu jobs generated all five summaries and the comparison job proved Linux = macOS = Windows byte-for-byte |
...
| OGVCS-001-AC-05 | Passed | Workspace, containment, no-replace publication, process-concurrency, schema, resource, persistence, and mutation tests passed; the hosted Windows-only portable-path/junction step also passed |
...
The first hosted Windows runs identified and drove fixes for the 100,001-operation timeout allowance, Git checkout CRLF conversion of canonical vectors, and direct `npm.cmd` spawning. Run 31863924721 passed after those gaps were fixed. The independent final review found no residual P0, P1, or P2 implementation or CI defect and approved OGVCS-001 for Done.
