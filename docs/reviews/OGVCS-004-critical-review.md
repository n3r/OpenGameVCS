...
| Windows package metadata | The source-package test treated npm's Windows file-mode metadata as POSIX executable authority. | Every host must contain and run the declared `bin`; POSIX additionally checks its source execute bits, while retained cross-OS archives normalize the bin member to mode `0755` before byte comparison. |
