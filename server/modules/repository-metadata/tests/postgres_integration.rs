...
fn migration_v1_v5_upgrade_report(database_url: &str) {
    ...
    assert_eq!((upgrade.applied, upgrade.already_applied), (36, 3));
    drop(store);
...
fn migration_report(database_url: &str) {
    ...
    assert_eq!(store.migrate(options).unwrap().applied, 39);
    assert_eq!(store.migrate(options).unwrap().already_applied, 39);
    drop(store);
    ...
    client
        .execute(
            "INSERT INTO ogvcs_metadata.schema_migrations
             (version, phase, checksum_sha256, state, minimum_application_version,
              maximum_application_version, completed_at)
             VALUES (14, 'expand', repeat('a', 64), 'completed', '0.2.0', '0.2.x', clock_timestamp())",
            &[],
        )
        .unwrap();
    drop(client);
    ...
    client
        .execute(
            "DELETE FROM ogvcs_metadata.schema_migrations WHERE version = 14",
            &[],
        )
        .unwrap();
}
