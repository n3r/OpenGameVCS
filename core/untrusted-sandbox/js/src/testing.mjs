...
const iterable = (chunks) => (async function* () { for await (const chunk of chunks) yield chunk; }());
export class FakeCandidateLauncher {
  constructor({ assertedControls = requiredControls(), stdout = ['{"schemaVersion":"ogvcs.untrusted-sandbox/parser-output/v1","outputs":[]}'], stderr = [], exit = Promise.resolve({ code: 0, signal: null }), start, terminate, kill } = {}) {
