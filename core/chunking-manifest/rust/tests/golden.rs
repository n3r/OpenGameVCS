...
const FRAGMENTATION: &str =
    include_str!("../../../../spec/chunking-manifest/v1/vectors/fragmentation.json");
...
fn assert_result(vector: &Value, bytes: &[u8], pattern: Option<&[usize]>) {
    let result = if let Some(pattern) = pattern {
        ...
        while offset < bytes.len() {
            ...
            chunker.update(&bytes[offset..offset + take]).unwrap();
            offset += take;
            cursor += 1;
        }
    ...
    let boundaries: Vec<u64> = expected["boundaries"]
        .as_array()
        .unwrap()
        .iter()
    ...
    assert_eq!(
        ...
        expected["logicalLength"].as_u64().unwrap()
    );
    assert_eq!(
        hex(&result.whole_file_digest),
        expected["wholeFileSha256"].as_str().unwrap()
    );
    assert_eq!(
        hex(&result.manifest.bytes),
        expected["manifestHex"].as_str().unwrap()
    );
    assert_eq!(
        result.manifest.object_id,
        expected["manifestObjectId"].as_str().unwrap()
    );
    for (actual, wanted) in result
        .parts
        .iter()
        .zip(expected["chunks"].as_array().unwrap())
    {
        assert_eq!(actual.length, wanted["length"].as_u64().unwrap());
        assert_eq!(actual.object_id, wanted["objectId"].as_str().unwrap());
    }
    let expected_digest: [u8; 32] =
        decode_hex(result.manifest.object_id.rsplit(':').next().unwrap())
            .try_into()
            .unwrap();
    assert_eq!(
        object_id(ObjectKind::ContentManifest, &result.manifest.bytes).unwrap(),
        expected_digest
    );
}
...
fn public_object_model_codec_matches_every_independent_golden_manifest() {
    ...
    assert_eq!(
        hex(&gear_table_sha256()),
        golden["tableSha256"].as_str().unwrap()
    );
...
fn update_fragmentation_does_not_change_boundaries_or_manifest() {
    ...
    for fixture in fragmentation["cases"].as_array().unwrap() {
        ...
        let vector = golden["cases"]
            .as_array()
            .unwrap()
            .iter()
            .find(|item| item["caseId"] == id)
            .unwrap();
        let bytes = source(&vector["recipe"]);
        for pattern in fixture["fragmentPatterns"].as_array().unwrap() {
            let sizes: Vec<usize> = pattern
                .as_array()
                .unwrap()
                .iter()
...
fn source_and_sink_failures_are_terminal() {
    assert!(matches!(
        Chunker::new(
            0,
            "chunking.opengamevcs/unknown@1",
            |_bytes, _part, _index| Ok(())
        ),
        Err(ChunkError::ProfileUnsupported)
    ));
    assert!(matches!(
        Chunker::new_with_resources(0, PROFILE, 1, 0, 2_097_151, |_bytes, _part, _index| Ok(())),
        Err(ChunkError::ResourceExhausted)
    ));
    assert!(matches!(
        Chunker::new_with_resources(0, PROFILE, 2, 0, 4_259_840, |_bytes, _part, _index| Ok(())),
        Err(ChunkError::ResourceUnsupported)
    ));
    let mut oversized = Chunker::new(67_108_865, PROFILE, |_bytes, _part, _index| Ok(())).unwrap();
    let oversized_fragment = vec![0; 67_108_865];
    assert_eq!(
        oversized.update(&oversized_fragment).unwrap_err(),
        ChunkError::FragmentInvalid
    );
    assert_eq!(
        oversized.update(&[]).unwrap_err(),
        ChunkError::SessionFailed
    );
    let mut short = Chunker::new(2, PROFILE, |_bytes, _part, _index| Ok(())).unwrap();
    short.update(&[1]).unwrap();
    assert_eq!(short.finish().unwrap_err(), ChunkError::SourceTooShort);

    let mut deliveries = 0;
    let mut failed = Chunker::new(2_097_152, PROFILE, |_bytes, _part, _index| {
        deliveries += 1;
        Err(ChunkError::SinkFailed)
    })
    .unwrap();
    let failed_fragment = vec![0; 2_097_152];
    assert_eq!(
        failed.update(&failed_fragment).unwrap_err(),
