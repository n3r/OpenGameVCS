...
use crate::{
    bundle_verify::{validate_bundle_logical_record_for_write, validate_bundle_object_for_write},
...
/// Validation failures before an item is emitted leave the writer retryable;
/// an I/O failure poisons it because the sink may contain a partial item.
...
impl<'a, W: Write> LogicalBundleWriter<'a, W> {
    ...
    pub fn write_object(&mut self, reference: ObjectRef, payload: &[u8]) -> Result<()> {
        ...
        let edges = if reference.kind == ObjectKind::Chunk {
            if object_id(reference.kind, payload)? != reference.digest {
            ...
            if opaque_object_digest(reference.kind.code(), payload)? != reference.digest {
                return Err(Error::new(ErrorCode::ObjectIdMismatch));
            }
            ...
            validate_bundle_object_for_write(
                actual_kind,
                scanned.value(),
                self.registry,
                self.operation,
    ...
    pub fn write_logical_record(&mut self, record: &Cbor) -> Result<TypedDigest> {
        ...
        let edges = validate_bundle_logical_record_for_write(
            record_type,
            record,
            self.registry,
            self.operation,
    ...
    pub fn write_object_root(&mut self, identity: ObjectRef, role: &ProfileRef) -> Result<()> {
        ...
        validate_write_assignment(
            self.registry,
            self.operation,
            RegistryAssignment::ObjectKind(identity.kind.code()),
        )?;
        validate_write_assignment(
            self.registry,
            self.operation,
        ...
        self.write_root(1, identity_bytes, role)?;
        self.object_roots += 1;
        Ok(())
    }
    ...
    pub fn write_logical_record_root(
        ...
        self.write_root(2, identity_bytes, role)?;
    ...
    fn write_root(&mut self, kind: u8, identity_bytes: Vec<u8>, role: &ProfileRef) -> Result<()> {
        ...
        validate_root_role(self.registry, self.operation, role)?;
