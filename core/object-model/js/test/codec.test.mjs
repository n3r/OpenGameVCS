...
import {
  ...
  validateBundleItem, validateConflictPreimage, validateLogicalRecord, validateRegistrySet, verifyLogicalBundle
} from '../src/index.js';
...
test('every normative proper-prefix truncation selects the declared layer-1 result', async () => {
  ...
  for (const range of recipe.wholeSequence.ranges) {
    for (let prefix = range.fromInclusive; prefix <= range.toInclusive; prefix += 1) {
      assert.throws(() => verifyLogicalBundle(sequence.subarray(0, prefix)), error => error instanceof OgvcsError &&
...
test('all metadata object vectors reproduce canonical bytes and ObjectIDs', async () => {
  ...
  for (const vector of index.objects) {
    ...
    if (vector.kind !== 1) {
      const decoded = decodeMetadata(payload, { registry, semantic: false });
...
test('all logical records reproduce canonical bytes and identities', async () => {
  ...
  for (const vector of index.records) {
    ...
    assert.equal(validateLogicalRecord(value, { registry }).type, vector.type);
...
test('all keyed conflict preimages reproduce their domain-separated IDs', async () => {
  ...
  for (const vector of index.combinations) {
    ...
    const value = decodeCanonical(payload);
    validateConflictPreimage(value, { registry });
    assert.equal(toHex(encodeCanonical(value)), toHex(payload));
...
test('current bundle item shapes round-trip without making closure claims', async () => {
  ...
  values.forEach(value => validateBundleItem(value));
...
test('typed references have exact binary parsers and canonical text forms', () => {
  ...
  expectCode(() => new ObjectRef(42, digest, new Map([[42, 'Bad:token']])), 'REGISTRY_INVALID');
...
test('unsupported required features remain scan/hash/forwardable but fail semantics', async () => {
  ...
  const old = new RegistrySnapshot({
    objectKinds: [...live.objectKinds.values()], profiles: [...live.profiles.values()],
    requiredFeatures: [], extensions: [...live.extensions.values()], limits: [...live.limits.values()],
    logicalRecordTypes: [...live.logicalRecordTypes.values()]
  });
  const newer = new RegistrySnapshot({
    objectKinds: [...live.objectKinds.values()], profiles: [...live.profiles.values()],
    requiredFeatures: [{ code: 1, name: 'vector-required-feature', state: 'ratified' }],
    extensions: [...live.extensions.values()], limits: [...live.limits.values()], logicalRecordTypes: [...live.logicalRecordTypes.values()]
  });
  const scanned = scanMetadata(payload, { registry: old });
  assert.equal(scanned.highestLayer, 1);
  assert.deepEqual(scanned.payload, payload);
  expectCode(() => decodeMetadata(payload, { registry: old }), 'REQUIRED_FEATURE_UNSUPPORTED');
  assert.equal(decodeMetadata(payload, { registry: newer }).highestLayer, 3);
  const unknownKind = encodeCanonical(new Map([[0, 1], [1, 65535], [2, []], [16, true]]));
  const opaque = scanMetadata(unknownKind, { registry: old });
  ...
  expectCode(() => decodeMetadata(unknownKind, { registry: old }), 'OBJECT_KIND_UNSUPPORTED');
});
...
test('unknown canonical optional extensions are byte-preserved losslessly', async () => {
  ...
  const decoded = decodeMetadata(payload, { registry });
  assert.equal(toHex(encodeCanonical(decoded.value)), toHex(payload));
...
test('framing forwards common-envelope schema defects until layer two', async () => {
  ...
  for (const [mutate, code] of cases) {
    ...
    assert.equal(scanMetadata(payload, { registry }).highestLayer, 1, code);
    assert.throws(
      () => decodeMetadata(payload, { registry, semantic: false }),
...
test('conformance profile rules run only at semantic layer three', async () => {
  ...
  assert.equal(decodeMetadata(payload, { registry, semantic: false }).highestLayer, 2);
  assert.throws(
...
test('all current profile-family positives and negatives route through shape validation', async () => {
  ...
  const validateArtifact = async artifact => {
    ...
    if (artifact.includes('bundle-root')) return validateBundleItem(value, { registry });
    if (artifact.includes('logical-records')) return validateLogicalRecord(value, { registry });
    return decodeMetadata(payload, { registry, semantic: false });
  };
  ...
  assert.equal(validateLogicalRecord(decodeCanonical(annotationB), { registry }).type, 8);
...
test('registry loading enforces canonical JSON and read/write state decisions', async () => {
  ...
  assert.equal(profileDecision(old, 'profile-state.test/ratified@1', 'new-write').state, 'ratified');
  ...
  expectCode(() => profileDecision(old, 'profile-state.test/deprecated@1', 'new-write'), 'PROFILE_STATE_FORBIDDEN');
...
test('tree basenames and opaque pending identifiers reject forbidden zero/dot forms', async () => {
  ...
  expectCode(() => validateLogicalRecord(pending), 'SCHEMA_FIELD_INVALID');
});
