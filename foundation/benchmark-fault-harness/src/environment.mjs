...
export function captureEnvironment(options) {
  ...
  const record = {
    ...
    implementation: { id: safeLabel(options.implementationId, 'ogvcs.reference/fake-service@1'), version: safeLabel(options.implementationVersion, '1.0.0-rc.1'), commit },
    corpus: { profileId: options.corpus.profile.id, profileVersion: options.corpus.profile.version, requestDigest: options.corpus.requestDigest, manifestDigest: options.corpus.manifestDigest, generatorVersion: '1.0.0' },
    configuration: { harnessVersion: '1.0.0-rc.1', harnessProfile: options.harnessProfile, thresholdDigest: options.thresholdDigest, seedDigest: canonicalDigest(options.seed, 'ogvcs.benchmark/seed/v1'), iterations: options.iterations, concurrency: options.concurrency, cacheState: options.cacheInspection.state, networkProfile: options.network.id },
