...
export class ProtocolSchemaValidator {
  ...
  constructor(schemas) {
    ...
    const entries = suppliedEntries.map(([name, schema]) => [name, deepFreeze(cloneJson(schema, {
  ...
  #auditSchema(schema, seen) {
    ...
    for (const [key, value] of Object.entries(schema)) {
      ...
      if (key === 'pattern') {
        ...
        try { this.#patterns.set(value, new RegExp(value, 'u')); } catch (error) {
    ...
    for (const key of direct) if (Object.hasOwn(schema, key) && (isObject(schema[key]) || typeof schema[key] === 'boolean')) this.#auditSchema(schema[key], seen);
    for (const key of ['allOf', 'anyOf', 'oneOf', 'prefixItems']) {
      if (schema[key] !== undefined) {
        if (!Array.isArray(schema[key])) protocolError(RUNTIME_ERROR_CODES.CONTRACT_INVALID, `${key} must be an array`);
        for (const child of schema[key]) this.#auditSchema(child, seen);
      }
    }
    for (const key of ['$defs', 'properties']) {
      if (schema[key] !== undefined) {
        ...
        for (const child of Object.values(schema[key])) this.#auditSchema(child, seen);
      }
    }
    seen.delete(schema);
  }
  ...
  schema(selector) {
    if (isObject(selector)) return selector;
  ...
  #validate(value, schema, root, path, state, evaluationDepth = 0) {
    ...
    if (typeof value === 'string') {
      ...
      if (schema.pattern !== undefined && !this.#patterns.get(schema.pattern).test(value)) throw new SchemaMismatch(path, 'does not match its pattern');
