...
- Implementation changes: private, unpublished, unwired Rust 1.82 source-projection preflight candidate in `core/import-git-lfs/rust`; it composes OGVCS-002 mapping/FileID types and OGVCS-004 path collision rules, separates unique Git blob objects from path occurrences/gitlinks, requires one path-bound mapping per non-gitlink occurrence, binds an explicit unauthenticated ordinary/required LFS disposition per occurrence, preserves symlink and ordinary-path bytes, classifies required canonical Git LFS v1 pointers with the official 1024-byte cutoff, verifies already-staged LFS bytes through a bounded no-I/O trait, and digest-binds a fully bounded in-memory report. The crate now also contains an independent strict decoder for one borrowed, already-inflated Git tree frame. The decoder verifies a caller-claimed staging SHA-256; performs an allocation-free structural/name-protection pass; admits literal work and peak logical retention; validates canonical Git ordering and global file/tree uniqueness with an exact-capacity bounded candidate stack; drops that scratch; then materializes exact output on a third pass. It rejects literal and Git-compatible HFS/NTFS `.git` aliases and binds its version, options, input digest, projection, and ledger. It is not connected to the preflight projection and does not recompute a Git object ID, decompress or acquire data, traverse history, authorize a request, run a sandbox job, map target paths/FileIDs, persist state, or publish refs.
- Test and benchmark results: Rust 1.82 debug, release, and packed fresh-consumer runs each pass 90 tests (47 preflight, 20 LFS, 10 typed-ID, 2 preflight known-answer, 9 tree-frame integration, 1 tree-frame commitment golden, and 1 internal phase-cancellation); strict all-target warnings-denied Clippy and formatting pass; Node source policy passes 4 tests; roadmap validation passes 8 tests; focused OGVCS-002 import predecessor checks pass 2 tests and OGVCS-004 path vectors pass 3 tests. Tree regressions cover both Git object widths, all supported modes, non-UTF-8 names, Git directory ordering, adjacent and non-consecutive duplicates, the upstream nested duplicate example, literal/HFS/NTFS `.git` aliases including malformed-UTF-8 end-marker behavior, malformed framing/modes/names/IDs, staged-digest and format substitution, exact/next hard and narrowed limits, work/retention admission, every cancellation phase and candidate-pop fencing, redaction, hostile deterministic inputs, replay, and independently generated commitments. Existing regressions cover the official LFS oversized-blob and marker matcher rules, reference-compatible extension case/underscore/suffix behavior, pointer-looking ordinary regular files, symlink filter bypass, one blob with different per-path LFS dispositions, null/sentinel commitments, same-blob/two-path distinct FileIDs and one-time byte charging, repeated-object conflicts, under/over/duplicate mappings, retry/new-state forgery, pre-source declaration admission, raw field capacity/length, all configurable hard ceilings, missing-finding precedence, LFS size/digest/EOF, generation drift, final cancellation, exhaustive limit/report binding including independently varied measured work and peak retention, and terminal failure. Exact revision `fa61786b272a019b82f4e96eaaa47dbef60c5b6c` passed this bounded source/package gate on hosted Linux, macOS, and Windows in [run 33664922211](https://github.com/n3r/OpenGameVCS/actions/runs/33664922211). This is source portability only; no Git repository conversion, large-history benchmark, performance target, or scale evidence is claimed.
- Security/reliability review: `docs/reviews/OGVCS-020-git-import-preflight-boundary-review.md` records the pure staged-input projection boundary, while `docs/reviews/OGVCS-020-git-tree-frame-boundary-review.md` records the separate one-frame decoder, staging-digest nonclaim, canonical byte semantics, three-pass bounded allocation boundary, cancellation/error atomicity, commitments, logical resource ledger, and unresolved authenticated acquisition, Git-ID, broker/sandbox, traversal, mapping, conversion, persistence, and publication work.
- Documentation/runbooks: `core/import-git-lfs/rust/README.md` documents strict Git LFS and inflated-tree semantics, unique-object/occurrence accounting, OGVCS-002 composition/nonclaims, limits, error precedence, residuals, and local gates. This is not an operator runbook, public import interface, or versioned production import format.
